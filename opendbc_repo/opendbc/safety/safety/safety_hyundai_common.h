...
// carrot: CC-only car (basic cruise, no SCC/radar). Lateral-only, no SCC12 rx required.
extern bool hyundai_cc_only;
bool hyundai_cc_only = false;
...
void hyundai_common_init(uint16_t param) {
  ...
  const int HYUNDAI_PARAM_CC_ONLY = 1024;
  ...
  hyundai_cc_only = GET_FLAG(param, HYUNDAI_PARAM_CC_ONLY);
